...
nll1<- function(pars, the.data, m0, pred.func){
  pp1<- pred.optim.Agt1.nls(pars, the.data, m0)
...
nll1(par.ml, growthdata, params.Agt1[4], pred.optim.Agt1.nls) 
#####################
...
nll1.wrap<- function(parwrap, A, the.data, pred.func){
  ...
  par[1]<- A # parameter A 
  return(nll1(par, the.data, params.Agt1[4], pred.func))
...
nll.ml<- nll1(par.ml[1:3], growthdata, par.ml[4], pred.optim.Agt1.nls)
...
while (nll.cl(A.low, growthdata, pred.optim.Agt1.nls, par.ml) <  0){
  A.up<- A.low
  A.low<- A.low - par.ml[1]
}

uu<- uniroot(nll.cl, c(A.low, A.up), growthdata, pred.optim.Agt1.nls, par.ml) 
...
while (nll.cl(A.up, growthdata, pred.optim.Agt1.nls, par.ml) <  0){
  A.low<- A.up
  A.up<- A.up  + par.ml[1]
}
...
par.start<- par.ml
for (A in A.vals){
  nll.temp<-  nll.prof(A, growthdata, pred.optim.Agt1.nls, par.start, return.pars=T)
